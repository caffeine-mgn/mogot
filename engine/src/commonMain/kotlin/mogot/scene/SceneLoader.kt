...
object SceneLoader {
    ...
    suspend fun loadScene(engine: Engine, stream: AsyncInputStream, loaders: List<NodeLoader>): Node {
        ...
        loadChilds(root, engine, stream, loadersMap)
        return root
    }

    private suspend fun loadChilds(root: Node, engine: Engine, stream: AsyncInputStream, loaders: Map<String, NodeLoader>) {
        ...
        (0 until childCount).forEach {
            ...
            val node = loader.load(engine, props)
            node.parent = root
            loadChilds(node, engine, stream, loaders)
        }
    }

    interface NodeLoader {
        val nodeClass: String
        suspend fun load(engine: Engine, props: Map<String, String>): Node
    }
}

suspend fun Resources.loadScene(path: String, loaders: List<SceneLoader.NodeLoader>): Node {
    val stream = fileSystem.get(Unit, "$path.bin")?.read() ?: throw FileSystem.FileNotFoundException(path)
    return stream.use {
        SceneLoader.loadScene(engine, it, loaders)
    }
}