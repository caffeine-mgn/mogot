...
object FbxImporter {
    ...
    fun import3(stream: InputStream): FbxMeta {

        val rootModel = FbxModel2(FbxRoot.Element("", emptyList(), listOf(FbxRoot.Element("Properties70", emptyList(), emptyList()))))
        val file = FbxRoot()
        ...
        file.get("Connections").single().get("C").forEach {
            ...
            val toObj = if (to == 0L) rootModel else objects[to]
        ...
        return FbxMeta(geoms = objects.values.mapNotNull { it as? FbxGeometry }, childs = rootModel.childs)
    }

    fun import(stream: InputStream): FbxModel2 {

        val root = FbxModel2(FbxRoot.Element("", emptyList(), listOf(FbxRoot.Element("Properties70", emptyList(), emptyList()))))
        val file = FbxRoot()
        FbxReader.read(stream, file)
        FBXTree.draw(file)
    ...
    fun import(gl: GL, obj: FbxModel2, material: () -> Material): Spatial {
        val node =
                if (obj.geometry != null)
                    GeomNode().also {
                        it.geom.value = import2(gl, obj.geometry!!)
                ...
                when (animation.type) {
                    AnimationCurveNode.Type.ROTATION -> {
                        animation.calc().forEach { time, vec ->
                            ...
                            anim.addRotation(time = (time * FbxRoot.SECONDS_PER_UNIT).toFloat(), rotation = f)
                        }
                    }
                    AnimationCurveNode.Type.POSITION -> {
                        animation.calc().forEach { time, vec ->
                            anim.addPosition(time = (time * FbxRoot.SECONDS_PER_UNIT).toFloat(), position = vec)
                        }
                    }

                    AnimationCurveNode.Type.SCALE -> {
                        animation.calc().forEach { time, vec ->
                            anim.addScale(time = (time * FbxRoot.SECONDS_PER_UNIT).toFloat(), scale = vec)
                        }
                    }
                }
            }
