...
actual interface Matrix4fc {
    fun get(array: Float32Array, offset: Int)
    actual fun getTranslation(dest: Vector3f): Vector3f
    val m00: Float
    val m01: Float
    val m02: Float
    val m03: Float
...
actual class Matrix4f : Matrix4fc {
    ...
    actual fun set(other: Matrix4fc): Matrix4f {
        m00 = other.m00
        m01 = other.m01
        m02 = other.m02
        m03 = other.m03
    ...
    actual fun translate(vector: Vector3fc): Matrix4f = translation(vector.x(), vector.y(), vector.z())
    ...
    actual fun rotate(other: Quaternionfc): Matrix4f =
            rotate(other, this)

    actual fun rotateAffine(other: Quaternionfc): Matrix4f =
            rotateAffine(other, this)

    actual fun scale(vector: Vector3fc): Matrix4f = scale(vector.x(), vector.y(), vector.z(), this)

    actual fun perspective(fovy: Float, aspect: Float, zNear: Float, zFar: Float): Matrix4f =
            perspective(fovy, aspect, zNear, zFar, this)

    override fun get(array: Float32Array, offset: Int) {
        array[0 + offset] = m00
        array[1 + offset] = m01
        array[2 + offset] = m02
        array[3 + offset] = m03
...
//        return sin(angle + PIHalf)
    ...
    var b: Double = a - (a / PI2) as Int * PI2
    if (b < 0.0) b += PI2
    return if (b >= kotlin.math.PI) -cos else cos
}*/

fun Matrix4fc.get(dest: Float32Array, offset: Int){
    dest[0 + offset] = m00
    dest[1 + offset] = m01
    dest[2 + offset] = m02
    dest[3 + offset] = m03
