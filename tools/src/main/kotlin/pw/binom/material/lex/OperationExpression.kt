...
class OperationExpression(
        ...
        override val source: SourcePoint) : Expression {
    ...
    enum class Operator(val innerMethod: String, val assignMethod: String) {
        ...
        companion object {
            val methods = Operator.values().map { it.innerMethod }
        ...
        fun read(lexer: LexStream<TokenType>): OperationExpression? =
                lexer.safe {
                    ...
                    OperationExpression(
                            ...
                            SourcePoint(
                                    lexer.module,
                                    left.source.position, right.source.position + right.source.length - left.source.position
                            )
                    )
                }
    }
